    ...
    @if (in_array($data->status_validasi, ['2', '3']))
        <div class="col-2">
            <a name="" id="" class="btn btn-success btn-lg"
                onclick="konfirmasi('update'+{{ $data->id_kemampuan_bahasa_asing }}+'bahasa','Apakah Anda Yakin ingin Menvalidasi data ini ?');"
                href="#" role="button">Validasi</a>
            <form
                action="{{ route('validasi.update', ['bahasa', $data->id_kemampuan_bahasa_asing]) }}"
                id="update{{ $data->id_kemampuan_bahasa_asing . 'bahasa' }}" method="post">
                @csrf
                @method('put')
            </form>
        </div>
    ...
    @if (in_array($data->status_validasi, ['1', '3']))
        <div class="col-2">
            <a name="" id="" onclick="tolakModal(this);"
                data-url="{{ route('validasi.destroy', ['bahasa', $data->id_kemampuan_bahasa_asing]) }}"
...
@elseif($type == 'wadek')
    <div class="col-2">
        <a name="" id="" class="btn btn-success btn-lg"
            ...
            href="#" role="button">Validasi</a>
        <form
            action="{{ route('validasi-wadek.update', ['bahasa', $data->id_kemampuan_bahasa_asing, $data->siakad_mhspt_id]) }}"
            id="update{{ $data->id_kemampuan_bahasa_asing . 'bahasa' }}" method="post">
            @csrf
            @method('put')
        </form>
    ...
    <div class="col-2">
        <a name="" id="" onclick="tolakModal(this);"
            data-url="{{ route('validasi-wadek.destroy', ['bahasa', $data->id_kemampuan_bahasa_asing]) }}"
            class="btn btn-danger btn-lg" href="#" role="button">Tolak</a>
    </div>
@endif
</div>
