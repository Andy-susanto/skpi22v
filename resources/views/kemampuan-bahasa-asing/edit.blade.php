...
@section('content_header')
    <h1 class="m-0 text-dark"><span><a name="" id="" class="btn btn-default btn-sm"
                href="{{ route('kemampuan-bahasa-asing.index') }}" role="button"><i class="fa fa-arrow-left" aria-hidden="true"></i> Kembali</a></span> Ubah Kemampuan Bahasa Asing <button type="button" class="btn btn-outline-primary btn-sm"><i class="fa fa-info" aria-hidden="true"></i> Detail</button></h1>
@endsection
    ...
    <div class="row">
        <div class="col-6 offset-3">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{route('kemampuan-bahasa-asing.update',encrypt($data['utama']->id_kemampuan_bahasa_asing))}}" method="post" enctype="multipart/form-data" id="form-seminar">
                        @csrf
                        @method('PUT')
                            ...
                            <select class="form-control" name="ref_bahasa_id" id="bahasa">
                                @forelse (Helper::Bahasa() as $bahasa)
                                    <option value="{{ $bahasa->id_ref_bahasa }}"
                                        {{ $data['utama']->ref_bahasa_id == $bahasa->id_ref_bahasa ? 'selected' : '' }}>
                                        {{ $bahasa->nama }}</option>
                                @empty
                            ...
                            <select class="form-control" name="ref_level_bahasa_id" id="level_bahasa" >
                                @forelse (Helper::level_bahasa() as $level_bahasa)
                                    <option value="{{ $level_bahasa->id_ref_level_bahasa }}"
                                        {{ $data['utama']->ref_level_bahasa_id == $level_bahasa->id_ref_level_bahasa ? 'selected' : '' }}>
                            ...
                            <select class="form-control" name="ref_jenis_tes_id" id="jenis_tes">
                                @forelse (Helper::jenis_tes() as $jenis_tes)
                                    <option value="{{ $jenis_tes->id_ref_jenis_tes }}"
                                        {{ $data['utama']->ref_jenis_tes_id == $jenis_tes->id_ref_jenis_tes ? 'selected' : '' }}>
                        ...
                        <div class="form-group">
                          ...
                          <input type="text" class="form-control" name="nilai_tes" id="" aria-describedby="helpId" placeholder="" value="{{$data['utama']->nilai_tes}}">
                        </div>
                        <div class="form-group">
                          <label for="">Bukti Kegiatan <span class="text-danger">*</span></label>
                          <input type="file" class="form-control-file" name="file_kegiatan_id" id="file_kegiatan" placeholder="" aria-describedby="fileHelpId">
                          <small id="fileHelpId" class="form-text text-muted"><a href="{{asset('storage/'.$data['utama']->files->path)}}"><i class="fa fa-paperclip" aria-hidden="true"></i> Bukti File Kegiatan</a></small>
                        </div>
                        <div class="row">
